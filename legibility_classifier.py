...
from networks import MyLegibilityConvNet
from flexible_networks import FlexibleLegibilityConvNet, AdaptiveConvNet, load_flexible_weights
...
def run(image_paths, model_path, threshold=0.5, arch='myconvnet'):
    # setup data
    ...
    if arch == 'resnet18':
        ...
        model_ft = LegibilityClassifierTransformer()
         ...
         model_ft = MyLegibilityConvNet()
    elif arch == 'simple':
         model_ft = LegibilitySimpleClassifier()
    elif arch == 'resnet34':
        model_ft = LegibilityClassifier34()
    elif arch == 'flexible':
        model_ft = FlexibleLegibilityConvNet()
    elif arch == 'adaptive':
        model_ft = AdaptiveConvNet()
    else:
        print(f"Warning: Unknown arch '{arch}' in legibility_classifier.run, defaulting to resnet34")
        model_ft = LegibilityClassifier34()

    if hasattr(state_dict, '_metadata'):
        del state_dict._metadata
    if arch == 'flexible' or arch == 'adaptive':
        model_ft = load_flexible_weights(model_ft, state_dict)
    else:
...
if __name__ == '__main__':
    ...
    parser.add_argument('--arch', choices=['resnet18', 'simple', 'resnet34', 'vit','myconvnet', 'flexible', 'adaptive'], default='myconvnet', help='what architecture to use')
    ...
    if args.arch == 'resnet18':
        ...
        model_ft = LegibilityClassifierTransformer()
    elif args.arch == 'myconvnet':
        print("Instantiating MyLegibilityConvNet")
        model_ft = MyLegibilityConvNet()
    ...
    if args.train or args.finetune:
        if args.finetune:
            ...
            if hasattr(state_dict, '_metadata'):
                del state_dict._metadata
            if args.arch == 'flexible' or args.arch == 'adaptive':
                model_ft = load_flexible_weights(model_ft, state_dict)
            else:
                model_ft.load_state_dict(state_dict)
