...
from networks import JerseyNumberClassifier 
...
python test_model.py --model_path models/baseline_ResNet_train_crops_epoch10_model.pth --image_dir out/SoccerNetResults/crops/imgs --output_file predictions.json --illegible_file out/SoccerNetResults/illegible.json --ground_truth_file data/SoccerNet/test/test_gt.json
#############################################################################################################################################################################
'''
...
def load_model(model_path, device):
    model = JerseyNumberClassifier()  # Initialize the correct model class
    model.load_state_dict(torch.load(model_path, map_location=device))  # Load model weights
    model.to(device)  # Send the model to the appropriate device (CPU or GPU)
...
def batch_inference(model, image_dir, output_file, device, illegible_file):
    ...
    for image_name in tqdm(image_list, desc="Processing Images", unit="image", dynamic_ncols=True):
    ...
    os.makedirs("BaseLineResNetResults", exist_ok=True)
    output_file_path = os.path.join("BaseLineResNetResults", output_file)
    ...
    # Create final_baseline_results by adding illegible results
    final_results = results.copy()

    # Save the final results as final_baseline_results.json
    final_output_file = os.path.join("BaseLineResNetResults", "final_baseline_results.json")
...
def mean_average_precision(y_true, y_pred, n_classes):
    ...
    for i in range(n_classes):
        ...
        try:
            ...
            pass  # Ignore classes with no samples
    
    return mAP / n_classes if n_classes > 0 else 0.0

def digit_level_accuracy(pred_digits, true_digits):
    correct_digits = sum(p == t for p, t in zip(pred_digits, true_digits))
    total_digits = len(true_digits)
    return correct_digits / total_digits if total_digits > 0 else 0.0
...
if __name__ == "__main__":
    ...
    evaluate_model("BaseLineResNetResults/final_baseline_results.json", args.ground_truth_file)
