...
class JerseyDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        try:
            ...
            return torch.zeros((3, 256, 256), dtype=torch.float32), jersey_num


class SwinTransformerV2:
    def __init__(self, model_size='small'):
        """Initialize Swin Transformer V2 model for jersey number classification

        Args:
            model_size: Size of the model, one of 'tiny', 'small', 'base', 'large'
        """
        # Determine device
        ...
        # Preprocessing for Swin V2 (224x224 is standard input size)
    ...
    def _build_model(self, num_classes):
        """Build Swin Transformer V2 model with given number of output classes"""
        # Get model name based on size
        model_name = self.model_map.get(self.model_size, 'swinv2_small_window16_256')

        print(f"Building {model_name} with {num_classes} output classes")
    ...
    def train(self, dir_to_jersey_mapping, batch_size=32, num_workers=16, epochs=5,
              learning_rate=5e-5, patience=5, weight_decay=0.05):
        """Train the Swin Transformer V2 model on augmented data

        Args:
            dir_to_jersey_mapping: Dictionary mapping directories to jersey numbers
            ...
            weight_decay: Weight decay for AdamW optimizer (default: 0.05)
        """
        ...
        # Loss function and optimizer - use AdamW for transformer models
        ...
        # Learning rate scheduler - Cosine Annealing works well with transformers
        ...
        best_model_path = os.path.join(model_dir, f"best_swinv2_{self.model_size}_jersey.pth")
        ...
        # Save final model
        final_model_path = os.path.join(model_dir, f"swinv2_{self.model_size}_jersey_classifier.pth")
    ...
    def _plot_training_history(self, history):
        ...
        # Save plot
        model_dir = 'D:\\ModelCheckpoints'
        plot_path = os.path.join(model_dir, f"swinv2_{self.model_size}_training_history.png")
        plt.tight_layout()
    ...
    def load_model(self, model_path=None, num_classes=100):
        ...
        if model_path is None:
            # Default to the saved path
            model_path = os.path.join('D:\\ModelCheckpoints', f"swinv2_{self.model_size}_jersey_classifier.pth")
