...
from networks import FPNResNet34  # Import the FPN-enhanced ResNet model
...
def load_model(model_path, device):
    model = FPNResNet34()  # Initialize the FPN-enhanced ResNet model
...
def batch_inference(model, image_dir, output_file, device, illegible_file):
    ...
    os.makedirs("FPNResNetResults", exist_ok=True)
    output_file_path = os.path.join("FPNResNetResults", output_file)
    ...
    # Create final_fpn_results by adding illegible results
    final_results = results.copy()

    # Save the final results as final_fpn_results.json
    final_output_file = os.path.join("FPNResNetResults", "final_fpn_results.json")
...
if __name__ == "__main__":
    ...
    evaluate_model("FPNResNetResults/final_fpn_results.json", args.ground_truth_file)