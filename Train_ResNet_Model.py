...
python Train_FPN_Model.py --train --data out_train\SoccerNetResults\crops --weights models
...
###########################################
'''
def train_fpn_model(model, optimizer, scheduler, num_epochs=10, lr=0.01):
    # Set device
...
###########################################
'''
def train_squeeze_model(model, optimizer, scheduler, num_epochs=10, lr=1e-3):
    # Set device
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    print("CUDA Available:", torch.cuda.is_available())
    print("Using device:", device)
...
if __name__ == '__main__':
    ...
    if args.train:
        ...
        if args.model_type == 'resnet34_multi':
            ...
            model_ft = train_squeeze_model(model_ft, optimizer_ft, exp_lr_scheduler, num_epochs=10, lr=0.01)
        elif args.model_type == 'fpn_resnet34':
